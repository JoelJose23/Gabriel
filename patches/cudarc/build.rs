...
fn main() {
    ...
    let (major, minor): (usize, usize) = if cfg!(feature = "cuda-12080") {
        ...
        #[cfg(feature = "cuda-version-from-build-system")]
        {
            ...
            // Clamp versions > 12.8 (e.g. CUDA 13.x) to 12.8 so rustc loads existing bindings
            let (cfg_major, cfg_minor) = if (major, minor) > (12, 8) {
                (12, 8)
            } else {
                (major, minor)
            };

            println!("cargo:rustc-cfg=feature=\"cuda-{cfg_major}0{cfg_minor}0\"");
            (major, minor)
        }
    };
